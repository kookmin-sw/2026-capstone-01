...
`FANOUT_MODE=node_channel` 모드에서 활성 노드 목록 (`chat:nodes` ZSET, score=만료시각ms) 을
추적. publisher 가 fan-out 시 이 레지스트리를 조회해 모든 노드 채널로 broadcast.

자가 치유: heartbeat 으로 만료시각 주기 갱신, 크래시 노드는 `list_active_nodes` 의
`ZREMRANGEBYSCORE` 로 자연 제거.
"""
...
async def _heartbeat_loop(stop_event: asyncio.Event) -> None:
    """`NODE_HEARTBEAT_INTERVAL_SEC` 마다 만료시각 갱신. 실패해도 계속 — 다음 tick 에 복구."""
    logger.info(
...
async def stop_node_registry() -> None:
    """앱 shutdown — heartbeat 루프 graceful 종료 + ZSET 에서 제거.
